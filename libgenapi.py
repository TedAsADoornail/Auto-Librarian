...
class Libgenapi(object):
    ...
    class __Scimag(object):
        ...
        def __parse(self, g):
            ...
            article = {"doi": None, "author": None, "article": None, "doi_owner": None, "journal": None,
                       "issue": {"year": None, "month": None, "day": None, "volume": None, "issue": None,
                                 "first_page": None, "last_page": None}, "issn": None, "size": None, "mirrors": []}
            i = 0
        ...
        def search(self, search_term="", journal_title_issn="", volume_year="", issue="", pages="", number_results=25):
            g = grab.Grab()
            request = {"s": search_term, "journalid": journal_title_issn, "v": volume_year, "i": issue, "p": pages,
                       "redirect": "0"}
            if sys.version_info[0] < 3:
                url = self.url + "?" + \
                      urllib.urlencode(request)
    ...
    class __Fiction(object):
        def __init__(self, url):
            self.url = 'https://libgen.rs/fiction'
            ...
            for resultRow in doc.select('/html/body/table[2]/tr'):
                ...
                for resultColumn in resultRow.select('td'):
                    if i > len(d_keys) - 1:
                        break
                    if d_keys[
                        i] == "libgenID_size_fileType_timeAdded_mirrors":  # Getting Libgen Id, size, fileType, time Added and mirror links.
    ...
    class __Comics(object):
        ...
        def __parse(self, g):
            ...
            comic = {"cover": None, "mirrors": [], "title": None, "size": None, "filetype": None,
                     "dateAdded": None, "scanDpi": None, "scanPixels": None, "pages_nfiles": None,
                     "pages_npictures": None, "comicsDOTorg": None}
            i = 0
            d_keys = ["cover", "mirrors", "http_mirror_and_title", "size_filetype", "dateAdded",
                      "scanDpi_and_scanPixels", "pages", "comicsDOTorg"]
            parse_result = []
            for resultRow in doc.select('/html/body/table[2]/tr'):
                i = 0
                comic = {"cover": None, "mirrors": [], "title": None, "size": None, "filetype": None,
