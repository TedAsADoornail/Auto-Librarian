...
def search_title_filtered(query, filters, exact_match=True):
...
def search_author_filtered(query, filters, exact_match=True):
    search_request = SearchRequest(query, search_type="author")
    results = search_request.aggregate_request_data()
    filtered_results = filter_results(
        results=results, filters=filters, exact_match=exact_match
...
def search_title(genre, query):
    search_request = SearchRequest(genre, query, search_type="title")
    return search_request.aggregate_request_data()
